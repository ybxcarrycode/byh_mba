...
public class CropImage {
    public String IMAGE_PATH = "Yunshuxie";
    public File FILE_SDCARD = Environment.getExternalStorageDirectory();
    public File FILE_LOCAL = new File(FILE_SDCARD, IMAGE_PATH);
    public File FILE_PIC_SCREENSHOT = new File(FILE_LOCAL, "pic");
    ...
    private Bitmap onSaveClicked(Bitmap bm) {
        ...
        int width = 600;// dr.width(); // modify by yc
        int height = 600;// dr.height();
        Bitmap croppedImage = Bitmap.createBitmap(width, height,
                Bitmap.Config.RGB_565);
        {
            Canvas canvas = new Canvas(croppedImage);
    ...
    public String saveToLocal(Bitmap bm) {
        ...
        String path = FILE_PIC_SCREENSHOT + "/yun" + NimApplication.user
                + "11" + String.valueOf((new Date()).getTime()) + ".jpg";
        try {
