...
public class PolyvUploadListActivity extends Activity implements OnClickListener {
	...
	private Button btn_uploadall, bt_select;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// initData();
		btn_uploadall.setOnClickListener(new OnClickListener() {
	...
	private void handle(Uri... uris) {
		...
		for (int i = 0; i < uris.length; i++) {
			...
			PolyvUploadInfo uploadInfo = new PolyvUploadInfo(filePath, fileName.substring(0, fileName.lastIndexOf(".")),
					"测试" + count);
	...
	// Toast.makeText(PolyvUploadListActivity.this,"第"+ (p + 1) +
	// "个任务文件不存在，或者大小为0", 0).show();
	// break;
	// case PolyvMTUploadVideo.FVIDEO:
	// Toast.makeText(PolyvUploadListActivity.this, "第"+ (p + 1) +
	// "个任务不是支持上传的视频格式", 0).show();
	// break;
	...
	// if (uris[i].toString().startsWith("content")) {
	// String[] filePathColumn = { MediaStore.Video.Media.DATA };
	// Cursor cursor = getContentResolver().query(uris[i], filePathColumn, null,
	// null, null);
	// cursor.moveToFirst();
	// int columnIndex = cursor.getColumnIndex(filePathColumn[0]);
	// filePath = cursor.getString(columnIndex);
	...
	// } else {
	// // 在文件中选择
	// filePath = uris[i].getPath().substring(uris[i].getPath().indexOf("/") +
	// 1);
	// }
	// count++;
	// File file = new File(filePath);
	// String fileName=file.getName();
	// PolyvUploadInfo uploadInfo = new
	// PolyvUploadInfo(filePath,fileName.substring(0,fileName.lastIndexOf(".")),
	// "测试" + count);
	// uploadInfo.setFilesize(file.length());
	// uploadInfo.setFilepath(filePath);
	// if (service != null && !service.isAdd(uploadInfo)) {
	// service.addDownloadFile(uploadInfo);
	// PolyvMThreadUploadManager.getPolyvUploader(filePath,
	// fileName.substring(0,fileName.lastIndexOf(".")) , "测试" + count);
	//
	// } else {
	// ((Activity) PolyvUploadListActivity.this).runOnUiThread(new Runnable() {
	//
	...
	// params1 = new LayoutParams(LayoutParams.WRAP_CONTENT,
	// LayoutParams.WRAP_CONTENT);
	// params2 = new LayoutParams(LayoutParams.MATCH_PARENT,
	// LayoutParams.WRAP_CONTENT);
	// params3 = new LayoutParams(LayoutParams.MATCH_PARENT,
	...
	// final int p = i;
	// // 设置监听
	// final PolyvMTUploadVideo mtuv =
	// PolyvMThreadUploadManager.getPolyvUploader(info.getFilepath(),
	// info.getTitle(),
	// info.getDesc());
	...
	// tv_filesize.setId(3);
	// ProgressBar pro_pro = new ProgressBar(this, null,
	...
	//
	// RelativeLayout.LayoutParams lp4 = new
	...
	//
	// RelativeLayout.LayoutParams lp5 = new
	// RelativeLayout.LayoutParams(params1);
	...
	//
	// RelativeLayout.LayoutParams lp6 = new
	// RelativeLayout.LayoutParams(params1);
	...
	//
	// RelativeLayout.LayoutParams lp7 = new
	// RelativeLayout.LayoutParams(params1);
	...
	// bt_down.setText("完成");
	// bt_down.setEnabled(false);
	// } else if(mtuv.isUploading())
	// bt_down.setText("暂停");
	// else
	// bt_down.setText("开始");
	// rl_downlist.addView(bt_down);
	//
	// RelativeLayout.LayoutParams lp8 = new
	...
	//
	// RelativeLayout.LayoutParams lp9 = new
	// RelativeLayout.LayoutParams(params1);
	...
	// dialog.setTitle("提示").setMessage("确认删除?").setPositiveButton("确定", new
