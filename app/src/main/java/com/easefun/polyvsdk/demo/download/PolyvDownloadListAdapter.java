...
import com.xszj.mba.utils.TimeUtils;
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.arg2 == NO_WRITE_PERMISSION) {
				Toast.makeText(context, "权限被拒绝，无法开始下载", Toast.LENGTH_SHORT).show();
				return;
			}
			Button btn = null;
			...
			case FAILURE:
				...
				switch (errorType) {
				case VID_IS_NULL:
					Toast.makeText(context, "第" + (position + 1) + "个任务vid错误，请重试", Toast.LENGTH_SHORT).show();
					break;
				case NOT_PERMISSION:
					Toast.makeText(context, "第" + (position + 1) + "个任务没有权限访问视频,请重试", Toast.LENGTH_SHORT).show();
					break;
				case RUNTIME_EXCEPTION:
					Toast.makeText(context, "第" + (position + 1) + "个任务运行时异常，请重试", Toast.LENGTH_SHORT).show();
					break;
				case VIDEO_STATUS_ERROR:
					Toast.makeText(context, "第" + (position + 1) + "个任务视频状态错误，请重试", Toast.LENGTH_SHORT).show();
					break;
				case M3U8_NOT_DATA:
					Toast.makeText(context, "第" + (position + 1) + "个任务m3u8没有数据，请重试", Toast.LENGTH_SHORT).show();
					break;
				case QUESTION_NOT_DATA:
					Toast.makeText(context, "第" + (position + 1) + "个任务问答没有数据，请重试", Toast.LENGTH_SHORT).show();
					break;
				case MULTIMEDIA_LIST_EMPTY:
					Toast.makeText(context, "第" + (position + 1) + "个任务ts下载列表为空，请重试", Toast.LENGTH_SHORT).show();
					break;
				case CAN_NOT_MKDIR:
					Toast.makeText(context, "第" + (position + 1) + "个任务不能创建文件夹，请重试", Toast.LENGTH_SHORT).show();
					break;
				case DOWNLOAD_TS_ERROR:
					Toast.makeText(context, "第" + (position + 1) + "个任务下载ts错误，请重试", Toast.LENGTH_SHORT).show();
					break;
				case MULTIMEDIA_EMPTY:
					Toast.makeText(context, "第" + (position + 1) + "个任务mp4下载地址为空，请重试", Toast.LENGTH_SHORT).show();
					break;
				case NOT_CREATE_DIR:
					Toast.makeText(context, "第" + (position + 1) + "个任务不能创建目录，请重试", Toast.LENGTH_SHORT).show();
					break;
				case VIDEO_LOAD_FAILURE:
					Toast.makeText(context, "第" + (position + 1) + "个任务Video加载失败，请重试", Toast.LENGTH_SHORT).show();
					break;
				case VIDEO_NULL:
					Toast.makeText(context, "第" + (position + 1) + "个任务video取得为null，请重试", Toast.LENGTH_SHORT).show();
					break;
				case DIR_SPACE_LACK:
	...
	private void initDownloaders() {
		for (int i = 0; i < data.size(); i++) {
			...
			downloader = PolyvDownloaderManager.getPolyvDownloader(_vid, info.getBitrate(),
					HlsSpeedType.getHlsSpeedType(info.getSpeed()));
	...
	public View getView(int position, View convertView, ViewGroup group) {
		...
		if (total != 0 && total == percent) {
			...
			holder.btn_download.setText("播放");
				...
				HlsSpeedType.getHlsSpeedType(info.getSpeed())).isDownloading())
		...
		public void onClick(View v) {
			...
			if (download.getText().equals("开始")) {
				...
				PolyvDownloader downloader = PolyvDownloaderManager.getPolyvDownloader(vid, bitRate,
						HlsSpeedType.getHlsSpeedType(speed));
				// 先执行
				if (notificationService != null) {
				...
				IjkVideoActicity.intentTo(context, IjkVideoActicity.PlayMode.portrait, IjkVideoActicity.PlayType.vid,
						vid, true, HlsSpeedType.getHlsSpeedType(speed), false);
			}
		}
