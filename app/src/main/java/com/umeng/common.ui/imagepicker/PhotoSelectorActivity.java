...
public class PhotoSelectorActivity extends Activity implements
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode !=0){
            ...
            onTakedPhoto(requestCode,resultCode);
        }
    }
    private void updateOkButton(){
        if (mSelectedPhotos.size() == 0){
            ...
            btnOk.setTextColor(ColorQueque.getColor("umeng_comm_small_title_color"));
        }else {
            btnOk.setSelected(true);
            btnOk.setText(ResFinder.getString("umeng_comm_next") + "(" + mSelectedPhotos.size() + "/" + MAX_IMAGE + ")");
            btnOk.setTextColor(ColorQueque.getColor("default_background_color"));
        }
...
//        ImagePickerUtils.launchActivity(this, PhotoPreviewActivity.class, bundle);
    ...
    protected void onTakedPhoto(int requestCode,int resultcode) {

        if (requestCode != TakePhotoPresenter.REQUEST_IMAGE_CAPTURE) {
            return;
        }
//            if (TextUtils.isEmpty( mPresenter.updateImageToMediaLibrary())){
//                return;
//            }
        Log.e("xxxxxxxx","requestCode="+requestCode+"  resultcode"+resultcode);
